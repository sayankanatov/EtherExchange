...
function vcGridFilterExcludeCallBack() {
    ...
    return $filterBy = $(".wpb_vc_param_value[name=filter_source]", this.$content), defaultValue = $filterBy.val(), $exclude = $(".wpb_vc_param_value[name=exclude_filter]", this.$content), autocomplete = $exclude.data("vc-param-object"), void 0 !== autocomplete && void $filterBy.change(function() {
        var $this = $(this);
...
function vc_wpautop(pee) {
    ...
    return pee.indexOf("<object") != -1 && (pee = pee.replace(/<object[\s\S]+?<\/object>/g, function(a) {
        return a.replace(/[\r\n]+/g, "")
    })), pee = pee.replace(/<[^<>]+>/g, function(a) {
        return a.replace(/[\r\n]+/g, " ")
    }), pee.indexOf("<pre") == -1 && pee.indexOf("<script") == -1 || (preserve_linebreaks = !0, pee = pee.replace(/<(pre|script)[^>]*>[\s\S]+?<\/\1>/g, function(a) {
        return a.replace(/(\r\n|\n)/g, "<wp-temp-lb>")
    })), pee.indexOf("[caption") != -1 && (preserve_br = !0, pee = pee.replace(/\[caption[\s\S]+?\[\/caption\]/g, function(a) {
...
function vcChartParamAfterAddCallback($elem, action) {
    if ("new" !== action && "clone" !== action || $elem.find(".vc_control.column_toggle").click(), "new" === action) {
        ...
        for (exclude = ["white", "black"], $select = $elem.find("[name=values_color]"), $options = $select.find("option"), i = 0;;) {
            if (100 < i++) break;
            if (random = Math.floor(Math.random() * $options.length), jQuery.inArray($options.eq(random).val(), exclude) === -1) {
...
function($) {
    ...
    _.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function() {
        ...
        return !!this.active_panel && void(model && vc.active_panel.model && vc.active_panel.model.get("id") === model.get("id") ? (vc.active_panel.model = null, this.active_panel.hide()) : model || (vc.active_panel.model = null, this.active_panel.hide()))
        ...
        save: function() {
            ...
            this.setAlertOnDataChange(), vc.$custom_css.val(this.editor.getValue()),
                vc.frame_window && vc.frame_window.vc_iframe.loadCustomCss(vc.$custom_css.val()), vc.updateSettingsBadge(), this.showMessage(window.i18nLocale.css_updated, "success"), this.trigger("save")
        },
        ...
        validateCellsList: function(cells) {
            ...
            return !(1e3 <= sum) && return_cells.join("_")
        }
...
function($) {
    ...
    vc.element_start_index = 0, vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
        ...
        createElement: function(e) {
            e && e.preventDefault && e.preventDefault();
            var model, column, row, showSettings, shortcode, row_params, inner_row_params, column_params, inner_column_params, tag;
            tag = $(e.currentTarget).data("tag"), row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {
                width: "1/1"
            }, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), !1 === this.model ? (vc.storage.lock(), "vc_section" === tag ? model = vc.shortcodes.create({
                shortcode: tag
                ...
                parent_id: row.id,
                root_id: row.id
            }), model = row, "vc_row" !== tag && (model = vc.shortcodes.create({
                shortcode: tag,
                parent_id: column.id,
                root_id: row.id
            })))) : "vc_row" === tag ? "vc_section" === this.model.get("shortcode") ? (vc.storage.lock(), row = vc.shortcodes.create({
                shortcode: "vc_row",
                params: row_params,
                parent_id: this.model.id,
                order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder()
                ...
                parent_id: row.id,
                root_id: row.id
            }), model = row) : (inner_row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row_inner && (inner_row_params = _.extend(inner_row_params, window.vc_settings_presets.vc_row_inner)), inner_column_params = {
                width: "1/1"
            }, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (inner_column_params = _.extend(inner_column_params, window.vc_settings_presets.vc_column_inner)), vc.storage.lock(), row = vc.shortcodes.create({
                shortcode: "vc_row_inner",
                ...
                parent_id: row.id,
                root_id: row.id
            }), model = row) : model = vc.shortcodes.create({
                shortcode: tag,
                parent_id: this.model.id,
                order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder(),
                root_id: this.model.get("root_id")
        ...
        createElement: function(e) {
            e && e.preventDefault && e.preventDefault();
            var showSettings, newData, i, shortcode, column_params, row_params, inner_row_params, inner_column_params, $control, tag;
            for ($control = $(e.currentTarget), tag = $control.data("tag"), row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row && (row_params = _.extend(row_params, window.vc_settings_presets.vc_row)), column_params = {
                    width: "1/1"
                }, "undefined" != typeof window.vc_settings_presets.vc_column && (column_params = _.extend(column_params, window.vc_settings_presets.vc_column)), this.prepend && (vc.activity = "prepend"), 0 == this.model ? "vc_section" === tag ? (this.builder.create({
                    shortcode: tag
                }), this.model = this.builder.last()) : (this.builder.create({
                    shortcode: "vc_row",
                    params: row_params
                }).create({
                    shortcode: "vc_column",
                    ...
                    shortcode: tag,
                    parent_id: this.builder.lastID()
                }), this.model = this.builder.last()) : "vc_row" === tag ? "vc_section" === this.model.get("shortcode") ? (this.builder.create({
                    shortcode: "vc_row",
                    params: row_params,
                    parent_id: this.model.id,
                    ...
                    params: column_params,
                    parent_id: this.builder.lastID()
                }), this.model = this.builder.last()) : (inner_row_params = {}, "undefined" != typeof window.vc_settings_presets.vc_row_inner && (inner_row_params = _.extend(inner_row_params, window.vc_settings_presets.vc_row_inner)), inner_column_params = {
                    width: "1/1"
                }, "undefined" != typeof window.vc_settings_presets.vc_column_inner && (inner_column_params = _.extend(inner_column_params, window.vc_settings_presets.vc_column_inner)), this.builder.create({
                    shortcode: "vc_row_inner",
                    params: inner_row_params,
                    parent_id: this.model.id,
                    order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.nextOrder()
                    ...
                    parent_id: this.model.id,
                    order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.nextOrder()
                }), this.model = this.builder.last()), i = this.builder.models.length - 1; i >= 0; i--) shortcode = this.builder.models[i].get("shortcode"), "undefined" != typeof window.vc_settings_presets[shortcode] && (this.builder.models[i].attributes.params = _.extend(this.builder.models[i].attributes.params, window.vc_settings_presets[shortcode]));
...
function($) {
    ...
    vc.ExtendPresets = {
        ...
        showDropdown: function() {
            window.vc_settings_show && this.$el.find('[data-vc-ui-element="settings-dropdown"]').show()
        },
        ...
        saveSettings: function(title, is_default) {
            ...
            if ("undefined" != typeof title && title.length) return "undefined" == typeof is_default && (is_default = !1), this.checkAjax(), this.ajax = $.ajax({
                type: "POST",
                dataType: "json",
                url: window.ajaxurl,
                ...
                response.success && (this.setSettingsMenuContent(response.html), this.settingsPresetId = response.id, is_default && (window.vc_settings_presets[shortcode_name] = this.getParamsForSettingsPreset()), this.untaintSettingsPresetData())
        ...
        fetchSaveSettingsDialog: function(callback) {
            ...
            return $contentContainer.find(".vc_ui-prompt-presets").length ? void("undefined" != typeof callback && callback(!1)) : (this.checkAjax(), void(this.ajax = $.ajax({
                type: "POST",
                dataType: "json",
                url: window.ajaxurl,
                data: this.fetchSaveSettingsDialogAjaxData()
            }).done(function(response) {
                response.success && ($contentContainer.prepend(response.html), "undefined" != typeof callback && callback(!0))
            }).fail(function() {
            ...
            this.isSettingsPresetDefault = !!is_default, this.fetchSaveSettingsDialog(function(created) {
                ...
                if ("undefined" !== window.vc_vendor_settings_presets[_this.model.get("shortcode")] ? $viewPresetsButton.removeAttr("disabled") : $viewPresetsButton.attr("disabled", "disabled"), $prompt.addClass("vc_visible"), $title.focus(), $contentContainer.addClass("vc_ui-content-hidden"), created) {
                    ...
                    $prompt.on("submit", function() {
                        var title = $title.val();
                        return !!title.length && (_this.saveSettings(title, _this.isSettingsPresetDefault).done(function() {
        ...
        saveAsDefaultSettings: function(id, doneCallback) {
            var shortcode_name = this.model.get("shortcode"),
                presetId = id ? id : this.settingsPresetId;
            !presetId || this.isSettingsPresetDataTainted() ? this.showSaveSettingsDialog(!0) : (this.checkAjax(), this.ajax = $.ajax({
                type: "POST",
                dataType: "json",
                url: window.ajaxurl,
                ...
                response.success && (this.setSettingsMenuContent(response.html), this.untaintSettingsPresetData(), window.vc_settings_presets[shortcode_name] = this.getParamsForSettingsPreset(), doneCallback && doneCallback())
            }).always(this.resetAjax))
        },
        ...
        restoreDefaultSettings: function() {
            ...
            this.checkAjax(), this.ajax = $.ajax({
                ...
                response.success && (this.setSettingsMenuContent(response.html), delete window.vc_settings_presets[shortcode_name], "undefined" !== window.vc_vendor_settings_presets[shortcode_name] && (window.vc_settings_presets[shortcode_name] = window.vc_vendor_settings_presets[shortcode_name]))
            }).always(this.resetAjax)
        },
        setSettingsMenuContent: function(html) {
            ...
            $button.data("vcShortcodeName", shortcode_name), $menu.html(html), window.vc_presets_data && window.vc_presets_data.presetsCount > 0 ? $menu.find("[data-vc-view-settings-preset]").removeAttr("disabled") : $menu.find("[data-vc-view-settings-preset]").attr("disabled", "disabled"), $menu.find("[data-vc-view-settings-preset]").on("click", function() {
                ...
                _this.deleteSettings($(this).data("vcDeleteSettingsPreset"))
        ...
        showViewSettingsList: function() {
            ...
            if ($contentContainer.find(".vc_ui-prompt-view-presets:not(.vc_visible)").remove(), !$contentContainer.find(".vc_ui-prompt-view-presets").length) {
                ...
                }), $prompt.off("click.vc2").on("click.vc2", "[data-vc-delete-settings-preset]", function(e) {
                    var $el = $(e.currentTarget);
                    _this.deleteSettings($el.data("vcDeleteSettingsPreset")) && ($el.closest(".vc_ui-template").remove(), $prompt.find(".vc_ui-template").length || closePrompt())
        ...
        return "vc_tta_section" === model.get("shortcode") && "undefined" != typeof params.tab_id && (params.tab_id = vc_guid() + "-cl"), params
    })
}(window.jQuery),
function($) {
    "use strict";
    vc.ExtendTemplates = {
        ...
        fetchSaveTemplateDialog: function(callback) {
            ...
            return $tab.find(".vc_ui-prompt-templates").length ? void("undefined" != typeof callback && callback(!1)) : (this.checkAjax(), this.ajax = $.ajax({
                type: "POST",
                dataType: "json",
                url: window.ajaxurl,
                data: this.fetchSaveTemplateDialogAjaxData()
            }).done(function(response) {
                response.success && ($tab.prepend(response.html), "undefined" != typeof callback && callback(!0))
            }).always(this.resetAjax), this.ajax)
        },
    ...
    vc.TemplateLibraryView = vc.PanelView.vcExtendUI(vc.HelperAjax).extend({
        ...
        getLibrary: function() {
            ...
            var data = this.getStorage("templates"),
                _this = this;
            data && "object" == typeof data && !_.isEmpty(data) ? (this.loaded = !0, this.loadLibrary(data), this.showLibrary()) : this.ajax = $.getJSON({
                url: "//vc-cc-templates.wpbakery.com/templates.json"
            }).done(function(data) {
        ...
        deleteTemplate: function(data) {
            if ("shared_templates" === data.type) {
                ...
                index !== -1 && (this.myTemplates.splice(index, 1), this.loaded && this.loadLibrary(this.data))
            }
        },
...
window.vc.filters = {
        ...
        var GoogleFonts = Backbone.View.extend({
            ...
            VC_AutoComplete = Backbone.View.extend({
                ...
                labelRemoveClick: function(e, ui) {
                    ...
                    var $label = $(e.currentTarget),
                        $target = $(e.target);
                    if ($target.is(".vc_autocomplete-remove")) return this.selected_items.splice($label.index(), 1), $label.remove(), this.updateItems(), this.$el.show(), !1
                },
        ...
        vc.debug = !1, vc.map = _.isUndefined(window.vc_mapper) ? {} : window.vc_mapper, vc.roles = _.isUndefined(window.vc_roles) ? {} : window.vc_roles, vc.Storage = function() {
            ...
            wrapData: function(content) {
                ...
                return content = _.reduce(matches, function(mem, value) {
                    value.trim().indexOf("vc_pseudo_section_") !== -1 || value.trim().match(starts_with_shortcode) || (value = "[vc_row][vc_column][vc_column_text]" + value + "[/vc_column_text][/vc_column][/vc_row]");
                    var matches_local = value.match(vc_regexp_shortcode());
                    return !_.isArray(matches_local) || _.isUndefined(matches_local[2]) || matches_local[2].indexOf("vc_pseudo_section_") !== -1 || _.isUndefined(vc.map[matches_local[2]]) || !_.isUndefined(vc.map[matches_local[2]].is_container) && vc.map[matches_local[2]].is_container || !_.isEmpty(vc.map[matches_local[2]].as_parent) || (value = "[vc_row][vc_column]" + value + "[/vc_column][/vc_row]"), mem + value
    ...
    function($) {
        ...
        vc.shortcode = Backbone.Model.extend({
            ...
            return dependencyMap = {}, defaults = {}, params = _.isObject(vc.map[tag]) && _.isArray(vc.map[tag].params) ? vc.map[tag].params : [],
            ...
            updateRowsSorting: function(e, ui) {
                _.defer(function(app) {
                    ...
                    var oldParentId = model.get("parent_id"),
                        $rows = parentNode.find(app.rowSortableSelector);
                    $rows.each(function() {
            ...
            setSortable: function() {
                return $(".wpb_main_sortable").sortable({
                    ...
                    over: function(event, ui) {
                        var tag = ui.item.data("element_type"),
                            parent_tag = ui.placeholder.closest("[data-element_type]").data("element_type") || "";
                        return vc.check_relevance(parent_tag, tag) && parent_tag != tag ? void ui.placeholder.css({
                    ...
                    stop: function(event, ui) {
                        ...
                        vc.check_relevance(parent_tag, tag) && parent_tag != tag || $(this).sortable("cancel");
                    ...
                    over: function(event, ui) {
                        var tag = ui.item.data("element_type"),
                            parent_tag = ui.placeholder.closest("[data-element_type]").data("element_type"),
                            allowed_container_element = !!_.isUndefined(vc.map[parent_tag].allowed_container_element) || vc.map[parent_tag].allowed_container_element;
                        vc.check_relevance(parent_tag, tag) && parent_tag != tag || ui.placeholder.addClass("vc_hidden-placeholder");
                        var is_container = _.isObject(vc.map[tag]) && (_.isBoolean(vc.map[tag].is_container) && !0 === vc.map[tag].is_container || !_.isEmpty(vc.map[tag].as_parent));
    ...
    function($) {
        ...
        window.VcRowView = vc.shortcode_view.extend({
            ...
            validateCellsList: function(cells) {
                ...
                return 12 === sum && return_cells.join("_")
            },
            ...
            addTab: function(e) {
                ...
                var tab_title = window.i18nLocale.tab,
                    tabs_count = this.$tabs.find("[data-element_type=tab]").length,
                ...
                /* nectar addition */
                 if(this.model.get('shortcode') === 'testimonial_slider') { 
                    ...
                    vc.shortcodes.create({shortcode:'testimonial', params:{title:tab_title, id:tab_id}, parent_id:this.model.id});
                 } else if(this.model.get('shortcode') === 'clients'){
                    tab_title = window.i18nLocale.client;
                    vc.shortcodes.create({shortcode:'client', params:{title:tab_title, id:tab_id}, parent_id:this.model.id});
                 } else if(this.model.get('shortcode') === 'nectar_icon_list'){
                    tab_title = window.i18nLocale.nectar_icon_list_item;
                    vc.shortcodes.create({shortcode:'nectar_icon_list_item', params:{title:tab_title, id:tab_id}, parent_id:this.model.id});
                    ...
                    vc.shortcodes.create({shortcode:'pricing_column', params:{title:tab_title, id:tab_id}, parent_id:this.model.id});
                    ...
                    vc.shortcodes.create({shortcode:'tab', params:{title:tab_title, id:tab_id}, parent_id:this.model.id});
                 }    
                 /* nectar addition end */
            ...
            updateSorting: function(event, ui) {
                ...
                }), vc.storage.unlock(), void vc.storage.save())
            },
            ...
            deleteShortcode: function(e) {
                var answer;
                return _.isObject(e) && e.preventDefault(), answer = confirm(window.i18nLocale.press_ok_to_delete_section), !0 === answer && (1 === vc.shortcodes.where({
                    parent_id: this.model.get("parent_id")
    ...
    function($, _) {
        ...
        function previewFilter(attachmentId) {
            ...
            }) : ($preview.attr("src", $preview.data("original-src")), void $previewContainer.removeClass("loading"))
        }
...
! function($) {
    ...
    vcPointersController = function(Pointer, texts) {
        ...
        build: function() {
            return this.pointer = this.getPointer(this._index), vc.events.once("backendEditor.close", this.close, this), !!this.pointer && void this.setShowEventHandler()
        },
        show: function() {
        ...
        dismissMessages: function() {
            return !this.messagesDismissed && ($.post(window.ajaxurl, {
...
function($) {
    ...
    vc.events.on("app.render", function() {
        ...
        this.pointer && $(this.pointer.target).length ? ($(this.pointer.target).parent().addClass("vc-with-vc-pointer-controls"),
            this.show(), $("#wpb_visual_composer").one("click", function() {
